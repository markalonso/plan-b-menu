...
export default function BottomSheet({ open, onClose, title, children }: BottomSheetProps) {
  ...
  return createPortal(
    <div
      className={cn('fixed inset-0 z-[200] isolate transition-opacity duration-calm ease-[cubic-bezier(0.22,1,0.36,1)] motion-reduce:transition-none', open ? 'pointer-events-auto opacity-100' : 'pointer-events-none opacity-0')}
      aria-hidden={!open}
    >
      {/* Backdrop */}
      <button
      ...
      <section
        ...
        className={cn(
          'absolute inset-x-0 bottom-0 z-10 mx-auto w-full max-w-3xl rounded-t-[28px] bg-surface shadow-elevate transition-transform duration-premium ease-[cubic-bezier(0.22,1,0.36,1)] will-change-transform motion-reduce:transition-none',
      ...
      >
        ...
        <div className="relative flex items-center justify-center pt-3 pb-2">
          ...
          <button
            className="absolute end-4 inline-flex h-8 w-8 items-center justify-center rounded-full text-sm text-muted transition-all duration-calm ease-[cubic-bezier(0.22,1,0.36,1)] hover:bg-surface2 hover:text-text active:scale-[0.95] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[color:var(--primary-focus-ring)] motion-reduce:transition-none motion-reduce:transform-none"
            onClick={onClose}
            aria-label="Close sheet"
          >
            ✕
